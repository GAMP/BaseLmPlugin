        ...
        protected void OnWait(Process process)
        {
            try
            {
                ...
                while (!this.IsWaitAborted)
                {
                    ...
                    var windowList = WindowEnumerator.ListWindows(process.Id);

                    //find desired window
                    IntPtr desiredWindow = (from win in windowList where string.Compare(win.Title, this.TargetWindowName, true) == 0 select win.Handle)
                        .FirstOrDefault();
                    ...
                    if (desiredWindow != IntPtr.Zero)
                    {
                        ...
                        //match pixels
                        if (CoreLib.Imaging.Imaging.HasMatchingPixels(applicationImage, pixelList, out int pixelsMatched))
                        {
